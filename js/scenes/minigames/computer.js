 ...
 *  ▣ 끝나는 기준(★): "제한 시간 안에 고정 개수(cm_budget=12) 다 닫기".
 *     - 진입 즉시 10개가 시간차로 쏟아짐(다다닥).
 *     - 창을 닫을 때마다 남은 예산(2개)에서 하나가 "또" 뜸 → 끄면 또 뜨는 짜증.
 *     - 제한 시간(cm_timeLimit초) 카운트다운. 0이 되기 전에 12개를 다 닫으면 승리,
 *       못 닫으면 시간 초과 → cm_onFail()(실제 게임에선 멸망 페널티/엔딩).
 *     - 상단 시간 게이지 + "닫은 창 X / 12" + 디지털 카운트다운을 노출.
 *
...
let cm_budget = 12;      // ★ 총 스폰 예산(닫아야 하는 창 개수)
let cm_burst = 10;       // 진입 시 다다닥 쏟아질 개수 (나머지 2개만 닫을 때 리스폰)
